...
#create_clock -period 6.4 -name clk_156mhz_int [get_nets clk_156mhz_int]
#set_clock_groups -asynchronous -group [get_clocks clk_156mhz_int] -group [get_clocks sfp_mgt_refclk_p]


# ============================================================================
# ADD THESE LINES TO THE END OF YOUR EXISTING XDC FILE
# These are the minimal constraints needed for LDPC clock domain crossing
# ============================================================================

# Clock domain crossing constraints for LDPC design
# These prevent timing violations between Ethernet and LDPC clock domains

# Set asynchronous clock groups (this is the most important constraint)
...
# False paths between clock domains to prevent timing analysis across async boundaries
set_false_path -from [get_clocks -filter {NAME =~ "*125*"}] -to [get_clocks -filter {NAME =~ "*ldpc*"}] -quiet
set_false_path -from [get_clocks -filter {NAME =~ "*ldpc*"}] -to [get_clocks -filter {NAME =~ "*125*"}] -quiet

# Max delay for async FIFO paths (helps with metastability)
set_max_delay 20.0 -from [get_clocks -filter {NAME =~ "*125*"}] -to [get_clocks -filter {NAME =~ "*ldpc*"}] -quiet  
set_max_delay 15.0 -from [get_clocks -filter {NAME =~ "*ldpc*"}] -to [get_clocks -filter {NAME =~ "*125*"}] -quiet

# Ignore timing on debug signals if you have ILA cores
#set_false_path -to [get_pins -hierarchical -filter {NAME =~ "*ila*"} -quiet]
